...
get_source_info() {
	...
	if [[ "$trackid" == *"firefox"* ]]; then
		...
		echo -e "Chrome  "
	elif [[ "$trackid" == *"plasma-browser-integration"* ]]; then
		echo -e "Plasma integration "
	else
		echo ""
	fi
}
...
--title)
	...
	if [ -z "$title" ]; then
		...
		echo "${title:0:120}" # Limit the output to 50 characters
	fi
	;;
--arturl)
...
--artist)
	...
	if [ -z "$artist" ]; then
		...
		echo "${artist:0:80}" # Limit the output to 50 characters
	fi
	;;
--length)
